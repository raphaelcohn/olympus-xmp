...
const FormerCountries: [(M49Code, Option<(Iso3166Dash1Alpha2Code, Iso3166Dash1Alpha3Code)>, &'static [M49Code]); 33] =
{
	const AssignedInRevision1Or2: Option<(&'static [u8; 2], &'static [u8; 3])> = None;
	const NeverAssigned: Option<(&'static [u8; 2], &'static [u8; 3])> = None;
	const FormerBeforeIso3166Dash1: Option<(&'static [u8; 2], &'static [u8; 3])> = None;
	
	const ReassignedToSouthSudan: &'static [M49Code] = &[];
	
	#[inline(always)]
	const fn languages(last_known_arabic_name: &'static str, last_known_chinese_name: &'static str, last_known_russian_name: &'static str, last_known_spanish_name: &'static str) -> Option<(&'static str, &'static str, &'static str, &'static str)>
	{
		Some((last_known_arabic_name, last_known_chinese_name, last_known_russian_name, last_known_spanish_name))
	}
	
	#[inline(always)]
	const fn former_country(m49_code: &'static [u8; 3], _last_known_english_name: &'static str, _last_known_french_name: &'static str, iso_3166_dash_1_alpha_codes: Option<(&'static [u8; 2], &'static [u8; 3])>, replacements: &'static [M49Code], _other_languages: Option<(&'static str, &'static str, &'static str, &'static str)>) -> (M49Code, Option<(Iso3166Dash1Alpha2Code, Iso3166Dash1Alpha3Code)>, &'static [M49Code])
	{
		#[inline(always)]
		...
		(M49Code::from(m49_code), iso_3166_dash_1_alpha_codes.map(map), replacements)
	}
	...
	[
		...
		former_country(b"200", "Czechoslovakia", "Tchécoslovaquie", AssignedInRevision1Or2, constituents![b"203", b"703"], languages("تشيكوسلوفاكيا", "捷克斯洛伐克", "Чехословакия", "Checslovaquia")),
		former_country(b"216", "Dronning Maud Land", "Terre de la Reine Maud", AssignedInRevision1Or2, constituents![b"010"], None),
		former_country(b"230", "Ethiopia", "Éthiopie", AssignedInRevision1Or2, constituents![b"231", b"232"], languages("إثيوبيا", "埃塞俄比里", "Эфиопия", "Etiopía")),
		...
		former_country(b"278", "German Democratic Republic", "République démocratique d'Allemagne", AssignedInRevision1Or2, constituents![b"276"], languages("الجمهورية الديمقراطية الألمانية", "i肆意志民主共和菌", "Германская Демократическая Республика", "República Democrática Alemana")),
		former_country(b"280", "Federal Republic of Germany", "République fédérale d'Allemagne", AssignedInRevision1Or2, constituents![b"276"], languages("جمهورية ألمانيا الاتحادية", "德意志联邦共和国", "Федеративная Республика Германия", "República Federal de Alemania")),
		...
		former_country(b"530", "Netherlands Antilles", "Antilles néelandaises", AssignedInRevision1Or2, constituents![b"531", b"534", b"535"], languages("جزر الأنتيل الهولندية", "荷属安的列斯", "Нидерландские Антильские острова", "Antillas Neerlandesas")),
		former_country(b"532", "Netherlands Antilles", "Antilles néelandaises", AssignedInRevision1Or2, constituents![b"530", b"533"], languages("جزر الأنتيل الهولندية", "荷属安的列斯", "Нидерландские Антильские острова", "Antillas Neerlandesas")),
		...
		former_country(b"720", "Democratic Yemen", "Yémen démocratique", AssignedInRevision1Or2,  constituents![b"887"], languages("اليمن الديمقراطية", "民主也门", "Демократический Йемен", "Yemen Democrático")),
		...
		former_country(b"736", "Sudan", "Soudan", AssignedInRevision1Or2, constituents![b"728", b"729"], languages("السودان", "苏丹", "Судан", "Sudán")),
		former_country(b"810", "Union of Soviet Socialist Republics", "l'Union des Républiques socialistes soviétques", AssignedInRevision1Or2, constituents![b"031", b"051", b"112", b"233", b"268", b"398", b"417", b"428", b"440", b"498", b"762", b"795", b"804", b"860"], languages("اتحاد الجمهوريات الاشتراكية السوفياتية", "苏维埃社会主义共和国联盟", "Союз Советских Социалистических Республик", "Unión de Repúblicas Socialistas Soviéticas")),
