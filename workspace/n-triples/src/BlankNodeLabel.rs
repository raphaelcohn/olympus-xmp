...
struct BlankNodeLabel<'a>(Cow<'a, str>);
...
impl<'a> BlankNodeLabel<'a>
{
	...
	fn parse<R>(remaining_bytes: &mut &'a [u8], constructor: impl FnOnce(Self) -> R) -> Result<R, BlankNodeLabelParseError>
	{
		...
		
		{
			...
			let character = decode_next_utf8(remaining_bytes)?.ok_or(DidNotExpectEndParsingFirstCharacterOfLabel)?;
			match character
			{
				...
				x00C0..=x00D6 | x00D8..=x00F6 | x00F8..=x02FF | x0370..=x037D | x037F..=x1FFF | x200C..=x200D | x2070..=x218F | x2C00..=x2FEF | x3001..=xD7FF | xF900..=xFDCF | xFDF0..=xFFFD | x10000..=xEFFFF => string.push_forcing_heap(character)?,
		...
		loop
		{
			...
			// `PN_CHARS_U ::= PN_CHARS_BASE | '_' | ':'`.
			// `PN_CHARS_BASE ::= [A-Z] | [a-z] | [#x00C0-#x00D6] | [#x00D8-#x00F6] | [#x00F8-#x02FF] |[#x0370-#x037D] | [#x037F-#x1FFF] | [#x200C-#x200D] | [#x2070-#x218F] | [#x2C00-#x2FEF] | [#x3001-#xD7FF] | [#xF900-#xFDCF] | [#xFDF0-#xFFFD] | [#x10000-#xEFFFF]`.
			let character = decode_next_utf8(remaining_bytes)?.ok_or(DidNotExpectEndParsingSubsequentCharacterOfLabel)?;
			match character
			{
				...
				x00B7 | x0300..=x036F | x203F..=x2040 => string.push_forcing_heap(character)?,
