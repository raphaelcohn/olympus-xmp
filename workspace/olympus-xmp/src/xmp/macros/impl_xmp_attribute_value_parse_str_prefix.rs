...
macro_rules! impl_xmp_attribute_value_parse_str_prefix
{
	($attribute: ty, $error_enum: ident, $prefix: literal, $($text: pat => $enum_member: ident, )*) =>
	{
		impl<'a> XmpAttributeValue<'a> for $attribute
		{
			...
			{
				let suffix = UnknownStringVariantParseError::parse_prefixed_value_returning_suffix::<$prefix>(raw)?;
				match suffix
				{
					$(
