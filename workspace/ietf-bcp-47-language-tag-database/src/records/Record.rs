...
impl Record
{
	...
	#[allow(missing_docs)]
	#[inline(always)]
	pub const fn deprecated(&self) -> Option<Date>
	{
		self.deprecated
	}
	...
	fn parse(events: &mut PullEventParser, records: &mut Records) -> Result<bool, LanguageSubtagRegistryFileContentsParseError>
	{
		let (type_, record_fields, no_more_records) = RecordFields::default().parse(events)?;
		
		let type_ = type_.ok_or(LanguageSubtagRegistryFileContentsParseError::MissingTypeField)?;
		
		Self::parse_record_from_fields(type_, records, record_fields).map_err(|cause| LanguageSubtagRegistryFileContentsParseError::Record(type_, cause))?;
